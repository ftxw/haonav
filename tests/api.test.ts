...
describe('api / icon（方案 B：前端直连 api.xinac.net，后端无代理路由）', () => {
  it('后端已无图标代理：/api/icon 与 /icon 均 404', async () => {
    const { app } = makeApp();
    expect((await app.request('/api/icon?url=https://github.com')).status).toBe(404);
    expect((await app.request('/icon?url=https://github.com')).status).toBe(404);
  });
});
