...
import { X, Save, Bot, Key, Globe, PauseCircle, Wrench, Copy, Check, LayoutTemplate, RefreshCw, Download, Sidebar, Keyboard, Package, Zap, Menu } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({
    isOpen, onClose, config, siteSettings, onSave
}) => {
...
chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.haonav_data) {
        refreshCache().then(buildMenus);
    }
});

async function refreshCache() {
    const data = await chrome.storage.local.get('haonav_data');
    if (data && data.haonav_data) {
        linkCache = data.haonav_data.links || [];
        categoryCache = data.haonav_data.categories || [];
    }
    return;
}
...
function updateMenuTitle(url) {
    ...
    // 仅更新标题，不重绘整个菜单，性能更高
    chrome.contextMenus.update("haonav_root", { title: newTitle }, () => {
